...
Report from <u><?php echo $StartDate; ?></u></strong></font> <strong><font color="#003399" size="5" face="Arial, Helvetica, sans-serif">to</font></strong> <u><font color="#003399"
 size="5" face="Arial, Helvetica, sans-serif"><strong><?php echo $EndDate; ?></strong></font></u>
	...
	*/
	...
	/*
	$sql = "SELECT tblProduct.ProductID, tblProduct.Color, tblProduct.Nickname, tblPurchase2.PurchaseID, tblPurchaseDetails2.PurchaseID,
	tblPurchaseDetails2.ProductID FROM tblPurchase2
	INNER JOIN tblPurchaseDetails2
	ON tblPurchase2.PurchaseID = tblPurchaseDetails2.PurchaseID
	INNER JOIN tblProduct
	ON tblProduct.ProductID = tblPurchaseDetails2.ProductID
...
<?
		...
		while($row = mysql_fetch_array($result))
				...
				$sql3 = "SELECT sum(tblShipmentProduct.Quantity) as NumShipped
				FROM tblPurchase2
				INNER JOIN tblPurchaseDetails2
				ON tblPurchase2.PurchaseID = tblPurchaseDetails2.PurchaseID
				INNER JOIN tblShipment3
				ON tblPurchase2.PurchaseID = tblShipment3.PurchaseID
				INNER JOIN tblShipmentProduct
				ON tblPurchaseDetails2.DetailID = tblShipmentProduct.DetailID
				WHERE tblPurchase2.OrderDateTime >= '$FromDate' AND tblPurchase2.OrderDateTime <= '$ToDate' AND
				tblShipment3.DateTime >= '$FromDate' AND tblShipment3.DateTime <= '$ToDate' AND tblPurchase2.Status = 'active'
				AND tblPurchaseDetails2.Status <> 'cancel' AND tblPurchaseDetails2.ProductID = '$ProductID'
				GROUP BY tblPurchaseDetails2.ProductID";
				
				//echo $sql3;
				
				$result3 = mysql_query($sql3) or die("cannot get NumOrdered");
...
?>	
 ...
 <?
	...
	tblPurchase2.Subtotal, tblPurchase2.Discount, tblPurchaseDetails2.ProductID, tblPurchase2.Shipped, tblProduct.Cost, tblProduct.BuiltInShipCost
	FROM tblPurchase2
	INNER JOIN tblPurchaseDetails2
	ON tblPurchase2.PurchaseID = tblPurchaseDetails2.PurchaseID
	INNER JOIN tblShipment3
