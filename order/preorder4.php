...
<form action="preorder3.php<? if($Custom=="yes"){echo "?a=$affiliateID";}?>" method="post" name="frmBack" id="frmBack">
  <div align="left">
...
<table width="100%" border="0" cellpadding="4" cellspacing="0" bordercolor="#003399">
 <tr><td width="40%"><p>	    <font size="2" face="Arial, Helvetica, sans-serif"><i>What you are being charged
      TODAY:</i></font></p>
	  <table width="100%" border="1" cellpadding="5" cellspacing="0" bordercolor="#003399">
        <tr>
          <td align="left" valign="top" bgcolor="#000099" colspan="4"><div align="center"><font color="#FFFFFF" size="3" face="Arial, Helvetica, sans-serif"><strong>Order
 ...
 # loop out all shopping cart items
        ...
        <tr>
          <td><div align="center" >
              <font size="2" face="Arial, Helvetica, sans-serif"><? echo $Num_Ordered; ?></font></div></td>
	...
	if($PromotionID == "")
	{
		...
		# -------------------------------------------------------------------------------
			...
			if($PromoType == "percent" AND $PromoAmount != 0)
			{
				...
				$Discount = $OriginalProductTotal * $PromoAmount;
	...
	else
	{
		...
		if($numPromotions > 0)
		{
			...
			while($i < $numPromotions) // loop through promotions in array
			{
				...
				{
					# person has a promotion to get a percent off of the total price... give it to them...
					$Discount = $OriginalProductTotal * $pAmount;
					$Discount = number_format($Discount, 2);
					...
					if ($txtState == 'TX')
						{
							...
							$row = mysql_fetch_array($result);
							$Tax =  $row[Value];
							$TaxTotal = ($OriginalProductTotal - $Discount + $ShipCost) * $Tax;
						}	
...
<form action="prereceipt2.php<? if($Custom=="yes"){echo "?a=$affiliateID";}?>" method="post" name="frmOrder" id="frmOrder" onSubmit="return lockButton(document.frmOrder.Confirm.value);">
		...
		<input name='ProductTotal' type='hidden' id='ProductTotal' value='<? echo "$ProductTotal";?>'>
		<input name='OriginalProductTotal' type='hidden' id='OriginalProductTotal' value='<? echo "$OriginalProductTotal";?>'>
		<!-- Shipping Address Info -->
		<input name='txtFirstName' type='hidden' id='txtFirstName' value='<? echo "$FirstName";?>'>
		<input name='txtLastName' type='hidden' id='txtLastName' value='<? echo "$LastName";?>'>
		<input name='txtAddress' type='hidden' id='txtAddress' value='<? echo "$Address";?>'>
