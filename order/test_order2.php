...
<form action="test_order1.php" method="post" name="frmBack" id="frmBack">
...
<form action="test_order3.php" method="post" name="frmorder3" id="frmorder3">
...
<form action="test_order3.php<? if($Custom=="yes"){echo "?a=$affiliateID";}?>" method="post" name="frmOrder" id="frmOrder" onSubmit="return CheckOrder();">
<table width="100%" border="0" cellpadding="2" cellspacing="0" >
...
<?
	...
	if ($Country != 225){ $sql .= " AND tblShipper.International = 'y'";}
	...
	if($DHLnum3 > '0' AND $USPSnum3 == 0 AND $POBox <> 'POBox')
		{
			if($Stampnum3 == 0)
			{
	...
	if($DHLnum3 == '0' AND $USPSnum3 == '0' AND $Stampnum3 > 0)
		{
		$sql .= " AND tblShippingOption2.ShippingOptionID = '9'";
		//echo "<br>10 = " .$sql;
		//$sql2 = " SELECT * FROM tblShippingCost6 WHERE ShipperID = $tempShipperID AND LocationID = $CountryID AND FromWeight <= $Weight AND ToWeight > $Weight AND ShippingOptionID != 10 AND Active = 'y'";
		//$sql2 .= " ORDER BY Cost";
		//echo "4 = " .$sql2;
		//$result2 = mysql_query($sql2) or die("Cannot get Shipping Options.  Please try again. $sql2");
		}
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$sql2 = "SELECT * FROM tblShippingCost6 WHERE ShipperID = '$tempShipperID' AND LocationID = '$Country' 
...
$ <? echo number_format($Cost,2); }?></strong></font></div></td>
			<td width="15%" valign="middle"><div align="center"  >		      <p>
			      <label>
...
<? if ( $ShippingCostID == "" ) { ?> <tr ><td colspan="3"><font face="Arial, Helvetica, sans-serif"><? echo "Currently there are no shipping options to accomodate your order. <br><br> Please contact a sales representative at "; ?> <font color="#FF0000"><? echo "800-552-0325" ; ?></font> or<font color="#FF0000"><? echo " 512-340-0338"; ?> </font> <? echo "so we can help you place your order."; ?></font></td>
  </tr> <? } ?></table>
<br>
<?
$Now = date("Y-m-d");
